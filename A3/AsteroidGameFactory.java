	...
	public Command MakeCommand(String commandLine)
	{
		...
		switch (commandToMake.toUpperCase())
		{
			...
			case "SPAWN_BUILDING":
			{
				int x = Integer.parseInt(args[0]);
				int y = Integer.parseInt(args[1]);
				BoardComponent building = GameBoard.Instance().GetBoard().get(y).get(x);
				return new SpawnBuildingCommand(building, args);
			...
			case "SPAWN_SHIELD":
			{
				int x = Integer.parseInt(args[0]);
				int y = Integer.parseInt(args[1]);
				BoardComponent shield = GameBoard.Instance().GetBoard().get(y).get(x);
				return new SpawnShieldCommand(shield, args);
